...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'GET') {
    ...
    const { customer_id, date, time, service_id, price } = req.body;

    if (!id || !customer_id || !date || !time || !service_id || price === undefined) {
    ...
    try {
      ...
      await client.sql`
        UPDATE appointments
        SET customer_id = ${customer_id}, date = ${date}, time = ${time}, service_id = ${service_id}, price = ${price}
        WHERE id = ${id}
      `;
